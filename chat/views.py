...
def download_excel(request, message_id):
    try:
        ...
        # 3. (ИСПРАВЛЕНО) Используем DatabaseExecutor и DataSource!
        # Ищем активный источник данных (так же, как в tasks.py)
        active_datasource = DataSource.objects.filter(is_active=True).first()

        # Инициализируем исполнителя с правильным источником (или дефолтным)
        db_executor = DatabaseExecutor(datasource=active_datasource)

        # 4. Повторно выполняем SQL (безопасно и в правильной БД)
        ...
        # Если была ошибка, редиректим назад, пользователь увидит сообщение об ошибке
